...
class MyNode(Node):
    ...
    def image_callback(self, msg):
        ...
        direcao = ctrl.Antecedent(np.arange(0, 3.14, 1), 'direcao')

        saida = ctrl.Consequent(np.arange(-3.14, 3.14, 0.001), 'saida')

        direcao.automf(number = 5, names = ['muitoEsquerda', 'Esquerda', 'Centro', 'Direita', 'muitoDireita'])

        saida.automf(number = 5, names=['muitoEsquerda', 'Esquerda', 'Centro', 'Direita', 'muitoDireita'])

        direcao['muitoEsquerda'] = fuzz.trimf(direcao.universe, [-0.5, -0.4, -0.3])
        direcao['Esquerda'] = fuzz.trimf(direcao.universe, [-0.25, -0.2, -0.1])
        direcao['Centro'] = fuzz.trimf(direcao.universe, [-0.0, 0.01, 0.02])
        direcao['Direita'] = fuzz.trimf(direcao.universe, [0.3, 0.4, 0.5])
        direcao['muitoDireita'] = fuzz.trimf(direcao.universe, [0.3, 0.4, 0.5])

        #direcao['muitoEsquerda'].view()

        saida['muitoEsquerda'] = fuzz.trimf(saida.universe, [-3.0, -2.5, -2.0])

        saida['Esquerda'] = fuzz.trimf(saida.universe, [-2.0, -1.5, -1.0])

        saida['Centro'] = fuzz.trimf(saida.universe, [0.0, 0.01, 0.02])

        saida['Direita'] = fuzz.trimf(saida.universe, [1.0, 1.5, 2.0])

        saida['muitoDireita'] = fuzz.trimf(saida.universe, [2.0, 2.5, 3.1])

        regra1 = ctrl.Rule(direcao['muitoEsquerda'], saida['muitoEsquerda'])
        regra2 = ctrl.Rule(direcao['Esquerda'], saida['Esquerda'])
        regra3 = ctrl.Rule(direcao['Centro'], saida['Centro'])
        regra4 = ctrl.Rule(direcao['Direita'], saida['Direita'])
        regra5 = ctrl.Rule(direcao['muitoDireita'], saida['muitoDireita'])

        sistema_controle = ctrl.ControlSystem([regra1, regra2, regra3, regra4, regra5])

        sistema = ctrl.ControlSystemSimulation(sistema_controle)

        sistema.input['direcao'] = angulo
        print("angulo: " + str(angulo))
        ...
        try:
            ...
            print("cal:" +  float(sistema.compute()))
            msg.twist.angular.z = float(sistema.compute()) #velocidade angular
